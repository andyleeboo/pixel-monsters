 ...
 *   node generate.mjs --count 6              6 random seeds
...
const svg = renderBanner(seeds, { slot, labels });
mkdirSync(dirname(out), { recursive: true });
writeFileSync(out, svg);
console.log(`${out}: ${seeds.length} monsters (seeds ${seeds.join(', ')}), ${svg.length} bytes`);
